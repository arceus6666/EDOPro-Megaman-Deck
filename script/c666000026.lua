...
function s.e1Target(e, tp, eg, ep, ev, re, r, rp, chk)
  ...
  if chk == 0 then
    return not Duel.IsPlayerAffectedByEffect(tp, CARD_BLUEEYES_SPIRIT) and
        Duel.IsPlayerCanSpecialSummonMonster(tp, id + 1, 0, TYPES_TOKEN, 0, 0, 1, RACE_WARRIOR, ATTRIBUTE_EARTH) and
        mx > 0
  end
...
function s.e1Operation(e, tp, eg, ep, ev, re, r, rp)
  ...
  if not Duel.IsPlayerAffectedByEffect(tp, CARD_BLUEEYES_SPIRIT) and
      Duel.IsPlayerCanSpecialSummonMonster(tp, id + 1, 0, TYPES_TOKEN, 0, 0, 1, RACE_WARRIOR, ATTRIBUTE_EARTH) and mx > 0
  then
    for i = 1, mx do
