...
function s.initial_effect(c)
  --fusion material
  c:EnableReviveLimit()
  Fusion.AddProcMix(c, true, true, PSEUDORIODS.PURPRILL, PSEUDORIODS.PROTECTOS)
...
s.listed_names = {
  BIOMETALS.BIOMETAL_P,
  PSEUDORIODS.PURPRILL,
  PSEUDORIODS.PROTECTOS,
  MEGAMEN.SIARNAQ
}
...
function s.filter(c)
  return c:IsCode(BIOMETALS.BIOMETAL_P) and c:IsAbleToHand()
end
...
function s.filter2(c, e, tp)
  return c:IsCode(MEGAMEN.SIARNAQ) and
